<template>
  <div class="pb-20">
    ...
    <h2
      class="px-6 mb-6 text-base font-rubik text-gray-500 md:text-black md:font-bold md:text-center md:px-0 md:text-2xl">
    ...
    <div class="flex flex-col justify-center space-y-6 md:space-y-0 md:space-x-6 md:flex-row md:px-6">
      <div class="w-full md:w-10/12">
        <div class="pt-6 bg-white pb-8">
          <NewRoomCalendar class="w-full" @selected="selectRooms($event)" :initialRooms="rooms"
            @viewsetup="closeTour()" />
          <!-- <hr /> -->
        ...
        <div class="flex gap-5 px-3">
          <button @click="clearBooking" class="w-64 bg-gray-button rounded-lg py-4 font-rubik text-sm font-medium">
            Restart Booking
          </button>
          <button @click="gotoNext()" class="w-64 bg-black text-white rounded-lg py-4 font-rubik text-sm font-medium">
            Continue
          </button>
        </div>
      ...
      <div class="flex-shrink-0 w-full px-3 md:px-0 md:w-3/12">
        <ReservationBox showDiscount :hideExtraGuests="false" />
      </div>
    </div>
    <div>
      <OvernightExtras ref="overnightExtrasRefs" />
    </div>
    <div class="flex gap-5 px-3">
      <button @click="clearBooking" class="w-64 bg-gray-button rounded-lg py-4 font-rubik text-sm font-medium">
        Restart Booking
    ...
    <div v-if="showExtraSelection"
      class="fixed inset-0 z-50 flex items-center justify-center w-screen h-screen bg-black bg-opacity-20">
      <div class="w-full max-w-lg p-12 bg-white rounded-md shadow-lg">
              ...
              <div class="text-sm">
                ...
                <div class="flex item-center" v-else-if="extraType == 'teen'">
                  <svg class="flex-shrink-0 w-4 h-4" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <circle stroke="#225A89" stroke-width=".8" stroke-linecap="round" stroke-linejoin="round" cx="25"
                      cy="26" r="2" />
                    <circle stroke="#225A89" stroke-width=".8" stroke-linecap="round" stroke-linejoin="round" cx="39"
                      cy="26" r="2" />
                    <path stroke="#225A89" stroke-width=".8" stroke-linecap="round" stroke-linejoin="round"
                ...
                <div class="flex item-center" v-else-if="extraType == 'small'">
                  ...
                  <div class="pl-2">Child</div>
                </div>
              </div>
              <select @change="addToExtraRooms($event.target.value, extraType, i)"
                class="w-full text-sm border border-gray-200 rounded-md outline-none focus:ring focus:outline-none"
        ...
        <div class="flex mx-auto mt-12 space-x-2 w-12/12 md:space-x-6 md:w-12/12 md:justify-end md:ml-auto md:items-end">
          ...
          <MainButton @click="selectionFinish(true)" v-if="Object.keys(extraRoomTypes).length >= extraPeople.total">Done
          </MainButton>
        </div>
      </div>
    </div>
...
import OvernightExtras from "~/components/daypass/OvernightExtras.vue";
...
export default {
  ...
  methods: {
    ...
    isSameRoomsAcrossDates() {
      ...
      const totalRooms = Object.keys(groupBy(this.rooms, "room_id")).length;

      let maxRoom = null;
      for (let i = 0; i < nights.length; i++) {
        const date = nights[i];

        const totalRoomsForDate = this.rooms.filter((r) => r.date == date).length;
    ...
    async gotoNext(skipExtras = false) {
      ...
      if (!this.isSameRoomsAcrossDates()) {
        this.$toast.error("Please select the same amount of rooms across each date");
        return;
      }
      ...
      this.$refs.overnightExtrasRefs.gotoNext();
