<template>
  <div class="pb-20">
    ...
    <div
      class="flex flex-col justify-center space-y-6 md:space-y-0 md:space-x-6 md:flex-row md:px-6"
    >
      <div class="w-full md:w-10/12">
        <div class="pt-6 bg-white pb-8">
          <NewRoomCalendar
            class="w-full"
            @selected="selectRooms($event)"
            :initialRooms="rooms"
            @viewsetup="closeTour()"
          />
          <!-- <hr /> -->
    ...
    >
      <div class="w-full max-w-lg p-12 bg-white rounded-md shadow-lg">
        ...
        <div class="grid grid-cols-2 mt-12 gap-x-6 gap-y-7">
          <template v-for="extraType in ['big', 'teen', 'small']">
            <div v-for="i in extraPeople[extraType]" :key="i + extraType">
              ...
              <select
                @change="addToExtraRooms($event.target.value, extraType, i)"
                class="w-full text-sm border border-gray-200 rounded-md outline-none focus:ring focus:outline-none"
                style="box-shadow: none"
              >
                <option value="">Select Room</option>
                <option
                  v-for="(room, ix) in uniqueRooms"
                  :key="ix"
                  :value="roomData[room.room_id].type"
                >
        ...
        <div
          class="flex mx-auto mt-12 space-x-2 w-12/12 md:space-x-6 md:w-12/12 md:justify-end md:ml-auto md:items-end"
        >
          <MainButton @click="selectionFinish(false)" outline
            >Back to Availability</MainButton
          >
          <MainButton
            @click="selectionFinish(true)"
            v-if="Object.keys(extraRoomTypes).length >= extraPeople.total"
            >Done</MainButton
          >
        </div>
      </div>
