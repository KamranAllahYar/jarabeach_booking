    ...
    <div class="px-4 md:px-0 border-b pb-10 mb-10 border-borderColor-light">
      ...
      <div class="flex gap-5 md:flex-row flex-col">
        <select
          ...
          id="adult"
          v-model="noOfAdult"
          class="border border-borderColor-light md:w-1/2 w-full rounded-xlg outline-none focus:outline-none p-3 text-gray-text font-rubik text-sm"
          style="box-shadow: none"
        >
          <option value="0" selected>Adults</option>
          <option v-for="num in 51" :value="num" :key="num">
            {{ num }}
          </option>
        </select>
        <select
      ...
      <div class="grid md:grid-cols-2 gap-4">
        <div v-for="(child, i) in childrenAges" :key="i" class="my-2 flex flex-col">
          <label for="" class="mb-4 font-normal text-xl font-rubik">Children’s age</label>
          <select
            v-if="noOfChildren"
            name="adult"
            id="adult"
            v-model="child.age"
            class="border border-borderColor-light rounded-xlg outline-none focus:outline-none p-3 text-gray-text font-rubik text-sm"
            style="box-shadow: none"
          >
			...
			<div class="border rounded-lg">
				<div class="p-4">
					...
					<div class="flex flex-col space-y-3 md:items-center md:flex-row md:space-x-6 md:space-y-0">
						<div class="flex items-center w-full pl-2 space-x-2 rounded-md md:w-auto focus-within:ring">
							<svg class="flex-shrink-0 w-5 h-5" viewBox="0 0 15 15" fill="none"
								xmlns="http://www.w3.org/2000/svg">
								<path
									d="M13.422 14.531a6.552 6.552 0 00-11.845 0M7.5 3.281a3.282 3.282 0 01-3.281 3.281 3.281 3.281 0 106.562 0A3.281 3.281 0 017.5 3.283v0z"
									stroke="#225A89" stroke-width=".8" stroke-linecap="round" stroke-linejoin="round" />
								<path
									d="M11.148 11.892a9.11 9.11 0 002.12-.63.938.938 0 00.476-1.225l-.86-2.006a2.813 2.813 0 01-.228-1.109V5.626a5.156 5.156 0 10-10.312 0v1.298c0 .38-.077.757-.228 1.108l-.86 2.006a.937.937 0 00.477 1.225 9.11 9.11 0 002.12.63"
									stroke="#225A89" stroke-width=".8" stroke-linecap="round" stroke-linejoin="round" />
							</svg>
							<div class="font-light">Adults</div>
							<select name="adult" id="adult" v-model="noOfAdult" class="w-full border-0 rounded-md outline-none focus:outline-none" style="box-shadow: none">
						...
						<div class="flex items-center w-full pl-2 space-x-2 rounded-md md:w-auto focus-within:ring">
							<svg class="flex-shrink-0 w-5 h-5" viewBox="0 0 16 16" stroke="currentColor" fill="none"
								xmlns="http://www.w3.org/2000/svg">
								<path
									d="M13.75 6.583h-.01C13.5 3.55 10.706 1.55 8 1.55s-5.5 2-5.74 5.033h-.01a1.75 1.75 0 100 3.5h.01a6 6 0 0011.48 0h.01a1.75 1.75 0 100-3.5z"
									stroke="#225A89" stroke-width=".8" stroke-linecap="round" stroke-linejoin="round" />
								<path
									d="M8 1.55a1.5 1.5 0 01.596 2.876 1.5 1.5 0 01-2.01-.876M9.323 11.334a2 2 0 01-2.646 0M11.5 7.333a1 1 0 01-2 0M6.5 7.333a1 1 0 01-2 0"
									stroke="#225A89" stroke-width=".8" stroke-linecap="round" stroke-linejoin="round" />
							</svg>

							<select v-model="noOfChildren" class="w-full border-0 rounded-md outline-none focus:outline-none" style="box-shadow: none">
				...
				<div class="p-4 border-t" v-if="noOfChildren">
					...
					<div class="grid w-full md:grid-cols-3 gap-x-3 gap-y-3">
						<div class="flex items-center pl-2 rounded-md focus-within:ring" v-for="(child, i) in childrenAges"
							:key="i">
							<svg class="flex-shrink-0 w-5 h-5" viewBox="0 0 16 16" stroke="currentColor" fill="none"
								xmlns="http://www.w3.org/2000/svg" v-if="child.age == '0 - 2'">
								<path
									d="M13.75 6.583h-.01C13.5 3.55 10.706 1.55 8 1.55s-5.5 2-5.74 5.033h-.01a1.75 1.75 0 100 3.5h.01a6 6 0 0011.48 0h.01a1.75 1.75 0 100-3.5z"
									stroke="#225A89" stroke-width=".8" stroke-linecap="round" stroke-linejoin="round" />
								<path
							...
							<svg class="flex-shrink-0 w-5 h-5" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg"
								...
								<circle stroke="#225A89" stroke-width=".8" stroke-linecap="round" stroke-linejoin="round"
									cx="25" cy="26" r="2" />
								<circle stroke="#225A89" stroke-width=".8" stroke-linecap="round" stroke-linejoin="round"
									cx="39" cy="26" r="2" />
								<path stroke="#225A89" stroke-width=".8" stroke-linecap="round" stroke-linejoin="round"
									d="M36.77 32.638l-1.54-1.276C34.354 32.418 33.207 33 32 33s-2.354-.582-3.23-1.638l-1.54 1.276C28.493 34.161 30.187 35 32 35s3.507-.839 4.77-2.362z" />
							</svg>
							<svg class="flex-shrink-0 w-5 h-5" viewBox="0 0 16 16" stroke="currentColor" fill="none"
							...
							<select placeholder="Select" required v-model="child.age"
								class="w-full border-0 rounded-md outline-none focus:outline-none" style="box-shadow: none">
...
export default {
  ...
  methods: {
    ...
    gotoNext() {
      this.$toast.clear();
      if (this.noOfAdult < 1) {
        this.$toast.info("Please let us know how many adults will be coming", {
          duration: 5000,
        });
        return;
      }
      ...
      if (this.noOfChildren > 0) {
        ...
        for (let ix = 0; ix < this.childrenAges.length; ix++) {
          if (this.childrenAges[ix].age == "") {
            this.$toast.info("Please let us know the age of each child", {
              duration: 5000,
            });
            return;
          }
