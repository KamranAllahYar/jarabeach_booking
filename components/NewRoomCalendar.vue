<template>
  <div class="flex flex-col w-full font-rubik">
    <div class="flex items-center px-2 mt-6 mb-6 space-x-4 md:px-6">
      <div class="flex items-center h-10 text-xs font-semibold md:text-lg text-black">
        {{ startDateStr }}
      ...
      <div class="flex-grow text-right">Click date twice for single night</div>
    </div>

    <div
      class="flex items-center justify-between px-3 py-3 text-xs md:text-base md:px-6 md:py-3"
    >
      <div class="flex items-center gap-6">
        <div class="flex items-center flex-shrink-0 text-black md:w-auto">
          <div
            class="flex items-center justify-center flex-shrink-0 w-4 h-4 mr-2 rounded-sm md:mr-4 md:w-8 md:h-8 bg-transparent"
          >
            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 19 19">
              <path
                d="M5.5415 3.1665L13.4582 15.8332"
                stroke="currentColor"
                stroke-width="1.25"
                stroke-linecap="round"
                stroke-linejoin="round"
              />
              <path
                d="M13.4582 3.1665L5.5415 15.8332"
                stroke="currentColor"
                stroke-width="1.25"
                stroke-linecap="round"
                stroke-linejoin="round"
        ...
        <div class="flex items-center text-black md:w-auto">
          <div
            class="text-center flex justify-center items-center flex-shrink-0 w-4 h-4 mr-2 rounded-full md:mr-4 md:w-8 md:h-8 bg-cal-non-avail"
          >2</div>
          Available - numbers represent available rooms
        </div>
      </div>
      <div>
        <a
          target="_blank"
          href="https://www.jarabeachresort.com/room-detail"
          class="self-end text-xs font-thin cursor-pointer md:text-base text-white bg-black rounded px-2 py-1"
          id="roomSetup"
        >
          View Room Setup
        </a>
    ...
    <div class="flex w-full" v-for="(datesHalf, k) in calType30Days" :key="k">
      ...
      <div class="flex-1 overflow-y-auto md:overflow-y-visible">
        <div class="flex md:grid mb-4" :class="`grid-cols-${datesHalf.length}`">
          <div
        ...
        <div
          class="flex md:grid"
          :class="`grid-cols-${datesHalf.length}`"
        ...
        >
          <div
            v-for="(compDate, ix) in datesHalf"
            :key="ix + '-df'"
            class="relative flex items-center justify-center flex-shrink-0 text-xl cursor-pointer font-extralight w-12 md:w-auto h-12 rounded-full"
            :class="
              roomsAvailable(roomType, compDate.dateStr) <= 0
                ? 'bg-transparent text-black'
                : 'bg-cal-non-avail text-black'
            "
            @click="selectRoom(roomType, compDate.dateStr)"
          >
            <!-- {{ compDate.dateStr }} -->
            <div
              v-if="isSingle(roomType, compDate.dateStr)"
              class="flex items-center justify-center w-full h-10 transform scale-110 bg-green-400 rounded-full"
            >
              <svg
            ...
            <div
              v-else-if="isStart(roomType, compDate.dateStr)"
              class="flex items-center justify-center w-full h-10 transform scale-110 bg-green-400 rounded-l-full"
            ...
            <div
              v-else-if="isEnd(roomType, compDate.dateStr)"
              class="flex items-center justify-center w-full h-10 transform scale-110 bg-green-400 rounded-r-full"
            >
              <svg
                class="w-6 h-6 text-white"
                fill="none"
            ...
            <div
              v-else-if="isBetween(roomType, compDate.dateStr)"
              class="w-full h-10 transform scale-110 bg-green-400"
            ></div>
            ...
            >
              ...
              >
                ...
                <div
                  ...
                  :class="popoverGridPatternClass"
                  v-else
                >
                  <template v-for="h in hoveredRooms">
                    <div
                      v-if="h.available == true"
                      :key="h.room.id"
                      class="flex items-center py-2 cursor-pointer"
                      @click="h.available == true ? addToBookedRoom(h.room, h.date) : ''"
                    >
                      <svg
                        v-if="!isBooked(h.room.id, h.date)"
                        viewBox="0 0 16 16"
                        class="flex-shrink-0 inline-block w-6 h-6 mr-2 text-brand-blue"
                        fill="none"
                        stroke="currentColor"
                      >
                        <path
                          d="M8 14.703a6.75 6.75 0 100-13.5 6.75 6.75 0 000 13.5z"
                          stroke="currentColor"
                          stroke-width="1.25"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                ...
                <div class="py-2 text-xs text-left" v-if="canSwap && shouldSwap">
                  <p>
                    ...
                    However, additional rooms may be available. Please proceed if you are
                    happy to change rooms during your visit
                  </p>
                  <button
                    class="flex items-center justify-end w-full pt-2 text-gray-800"
                    @click="selectIndividualRooms()"
                  >
            ...
            <template v-if="isEnd(roomType, compDate.dateStr) && showMobileSelect">
              <div
                class="fixed inset-0 z-40 flex items-end w-screen h-screen bg-black bg-opacity-5"
                @click.stop
              >
                <div class="flex-1"></div>
                <div
                  class="w-full pt-3 pb-12 bg-white"
                  @click.stop
                  v-if="showRoomSelect && canSwap"
                >
                ...
                <div class="w-full px-4 pt-6 pb-12 bg-white" @click.stop v-else>
                  ...
                  <div v-else>
                    <template v-for="h in hoveredRooms">
                      <div
                        ...
                        class="flex items-center py-2 cursor-pointer"
                        @click="
                          h.available == true ? addToBookedRoom(h.room, h.date) : ''
                        "
                      >
                        <svg
                          v-if="!isBooked(h.room.id, h.date)"
                          viewBox="0 0 16 16"
                          class="inline-block w-6 h-6 mr-2 text-brand-blue"
                          fill="none"
                          stroke="currentColor"
                        >
                          <path
                  ...
                  <div class="py-2 text-xs text-left" v-if="canSwap && shouldSwap">
                    <p>
                      We are able to welcome you on your chosen dates in the rooms above.
                      However, additional rooms may be available. Please proceed if you
                      are happy to change rooms during your visit
                    </p>
                    <button
                      class="flex items-center justify-end w-full pt-2 text-gray-800"
                      @click="selectIndividualRooms()"
    ...
    <div class="flex items-center justify-center py-3 bg-gray-100 md:hidden">
      <svg width="44" height="31" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
          d="M20.84 17.29l-3.158-2.577a1.365 1.365 0 01-.471-1.397v0a1.366 1.366 0 011.934-.89l.978.489V7.29a1.25 1.25 0 011.25-1.25v0a1.25 1.25 0 011.25 1.25v3.75l1.661.278a2.5 2.5 0 012.09 2.465v3.507M12.623 6.04h4.375M15.748 2.915L12.623 6.04l3.125 3.125M25.123 6.04h6.25M28.248 2.915l3.125 3.125-3.125 3.125"
          stroke="#747474"
          stroke-linecap="round"
          stroke-linejoin="round"
        />
...
export default {
  ...
  computed: {
    ...
    isMaxMonthYear() {
      return this.calYear >= this.maxYear && this.calMonth >= this.maxMonth;
    },
    isMinMonthYear() {
      return this.calYear <= this.minYear && this.calMonth <= this.minMonth;
    },
    bigPeople() {
  ...
  methods: {
    ...
    isSingle(roomType, dateStr) {
      return this.isStart(roomType, dateStr) && this.isEnd(roomType, dateStr);
    },
    ...
    roomTypeIds(roomType) {
      return this.rooms.filter((room) => room.type == roomType).map((room) => room.id);
    },
    dayOfWeek(day) {
      return this.weekDays[getDay(new Date(this.calYear, this.calMonth - 1, day))];
    },
    nextMonth() {
    ...
    async getRoomsAvailableForPeriod() {
      ...
      return await this.$axios
        ...
        .then((res) => {
          ...
          if (this.seRoom == "family") {
            this.hoveredRooms.sort((a, b) => (a.type > b.type ? 1 : -1));
          }
        })
        .finally(() => {
