  ...
  <div class="flex flex-col w-full md:flex-row">
    ...
    <div class="w-full p-6 md:w-7/12">
      ...
      <p class="mt-3 font-light leading-relaxed text-gray-600">
        There are a number of expected extras we offer for free, including romantic room decoration and happy
        birthday messages. Add balloon decoration below for just N5,000.
        <br />
        <br />Helium Balloons or bespoke Special Decoration on request (please email
        <a class="text-brand-blue" href="mailto:bookings@jarabeachresort.com">bookings@jarabeachresort.com</a>)
      </p>
          ...
          <div v-for="deco in decorations" :key="deco.id">
            ...
            <template v-else>
              ...
              <div class="md:ml-8" v-if="showNote(deco)">
                <input type="text" :key="deco.id + 'ss'" placeholder="What to write. 20 characters max" maxlength="20" v-model="myWelcomeNote" class="w-full py-1 rounded-lg focus:outline-none focus:ring focus:ring-brand-blue-300 ring-offset-4" />
              </div>
...
export default {
  ...
  mounted() {
    ...
    if (this.dates.length > 0) {
      ...
      this.breakfastDates.push(format(add(parseISO(lastDate), { days: 1 }), 'yyyy-MM-dd'));
    ...
    if (this.$store.state.extras.selectedDecorations) {
      this.selectedDecorations = this.$store.state.extras.selectedDecorations.map(x => x);
    }
