    ...
    <div class="flex flex-col w-full md:flex-row md:h-640">
        ...
        <div class="flex flex-col w-full h-full py-6 md:w-7/12">
            <div class="flex-1 px-6 pb-4 md:overflow-y-auto" id="con_scroll">
                ...
                <div class="mt-3">
                    <div class="flex flex-wrap items-center mt-0 space-y-4 divide-y justify-betweenl md:space-y-3">
                        <div class="flex flex-col w-full py-3 md:flex-row md:items-center" v-for="(sMassage, ix) in selectedMassages" :key="ix">
                            <div class="flex flex-col items-center w-full space-x-0 space-y-2 font-light md:space-y-4 md:items-start">
                                <div class="w-full">
                                    ...
                                    <div class="flex items-center flex-1 w-full border rounded-md focus-within:ring">
                                        <select v-model="sMassage.id" class="w-full text-sm border-0 rounded-md outline-none focus:outline-none" style="box-shadow: none">
                                    ...
                                    <div class="flex w-full space-x-3">
                                        <div class="flex-1">
                                            <div class="flex items-center flex-1 border rounded-md focus-within:ring">
                                                <select placeholder="Date" v-model="sMassage.date" class="w-full text-sm border-0 rounded-md outline-none focus:outline-none" style="box-shadow: none">
                                                    <option>Date</option>
                                        ...
                                        <div class="flex items-center flex-shrink-0 border rounded-md focus-within:ring">
                                            <select placeholder="Time" v-model="sMassage.time" class="w-full text-sm border-0 rounded-md outline-none focus:outline-none" style="box-shadow: none">
                                                <option>Time</option>
                                                <option v-for="time in getTimeOptions(sMassage.date)" :value="time" :key="time">
                                                    {{ time }}
                                                </option>
                                            </select>
                                        </div>
                            ...
                            <div @click="removeDrink(ix)" v-if="selectedMassages.length > 1" class="flex items-center flex-1 flex-shrink-0 h-10 mt-3 text-xs cursor-pointer md:ml-5 md:mt-0 text-brand-red hover:text-red-300">
                                <div>Remove</div>
                            </div>
                        </div>
                    </div>

                    <div @click="addMassage" v-if="selectedMassages.length < 2" class="flex items-center flex-shrink-0 h-10 mt-3 text-xs cursor-pointer w-44 text-brand-blue hover:text-brand-blue-300">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                        </svg>
                        <div>Add Another Massage</div>
                    </div>
                </div>
            </div>
            <div class="border-t">
