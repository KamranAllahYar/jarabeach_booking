<template>
	<div class="flex flex-col w-full md:flex-row md:h-640">
		<div class="relative w-full md:h-auto md:w-6/12 h-60">
			<img src="@/assets/images/specials/conferences.jpeg" alt class="object-cover object-center w-full h-full" />
		</div>
		<div class="flex flex-col w-full h-full py-6 md:w-7/12">
			<div class="flex-1 px-6 pb-4 md:overflow-y-auto" id="con_scroll">
				<div class="font-semibold">Conference Facility</div>
				<p class="mt-3 font-light leading-relaxed text-gray-600">
					Exclusive Use of the Gazebo, includes tea/Coffee, seating, big screen & Stationary
				</p>
...
export default {
	...
	methods: {
		next() {
			...
			this.$store.commit('extras/SET_SELECTED_CONFERENCES', this.selectedDates);
			this.$emit('next');
		},
		prev() {
			this.$store.commit('extras/SET_SELECTED_CONFERENCES', this.selectedDates);
      ...
      this.$axios.post('/check-conference-booking', {"dates": this.bookingDates}).then(res => {
        this.noDates = this.noDates.concat(res.data.data)

        if (this.dates.length == 0) {
          this.$toast.info("Conference Facility is not available during your stay")
        }
	...
	created() {
		this.getNoConferenceDates();
	},
	mounted() {

		if (this.$store.getters["extras.allSelectedConferenceDates"]) {
			this.selectedDates = this.$store.getters["extras.allSelectedConferenceDates"];
		}
