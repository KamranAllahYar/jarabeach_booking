<template>
  <div class="flex flex-col w-full md:flex-row md:h-auto">
    <div class="relative w-full md:h-auto md:w-6/12 h-60">
      <img
        src="@/assets/images/specials/bikes.png"
        alt
    ...
    <div class="flex flex-col w-full h-full py-6 md:w-7/12">
      <div class="flex-1 px-6 pb-4 md:overflow-y-auto" id="con_scroll">
        <div class="font-semibold">Horse Riding</div>
        ...
        <p class="mt-2 font-light leading-relaxed text-gray-600">
          Horse riding is available through local horse owners, along the beachfront. A
          riding waiver / terms of usage must be signed, prior to riding. PLEASE BE AWARE
          riders do so at their own risk. Suitable for Adult riders [only] and passengers
          (under 18yrs with signed guardian).
        </p>

        <div class="mt-3">
          <div
            v-if="showAddButton"
            class="flex flex-wrap items-center justify-between mt-0 space-y-4 divide-y md:space-y-3"
          >
            <div class="flex flex-col w-full py-3 md:flex-row md:items-center">
              <div
                class="flex flex-col items-center w-full space-x-0 space-y-2 font-light md:space-y-4 md:items-start"
              >
                <div class="w-full">
                  <div class="pb-1 text-sm font-bold">Please select a package</div>
                  <div
                    class="flex items-center flex-1 w-full border rounded-md focus-within:ring"
                  >
                    <select
                      v-model="newBike.id"
                ...
                <div class="w-full">
                  <div class="pb-1 text-sm font-bold">
                    On what date would you like this service?
                  </div>
                  <div class="flex w-full space-x-3">
                    <div class="flex-1">
                      <div
                        class="flex items-center flex-1 border rounded-md focus-within:ring"
                      >
                        <select
                          placeholder="Date"
                          v-model="newBike.date"
                          class="w-full text-sm border-0 rounded-md outline-none focus:outline-none"
                          style="box-shadow: none"
                        >
                          <option>Date</option>
          ...
          <div
            @click="addNewMassage"
            v-if="showAddButton"
            class="flex items-center flex-shrink-0 h-10 mt-0 text-xs cursor-pointer w-44 text-brand-blue hover:text-brand-blue-300"
          >
            <svg
              class="w-4 h-4"
              fill="none"
          ...
          <div
            class="flex flex-wrap items-center justify-between mt-4 space-y-4 md:space-y-3"
          >
            <div
              class="flex flex-col w-full px-3 py-2 bg-gray-100 rounded md:flex-row md:items-center"
              v-for="(sMassage, ix) in selectedBikes"
              :key="ix"
            >
              <div class="w-full">
              ...
              <button
                @click="removeMassage(ix)"
                class="flex items-center flex-1 flex-shrink-0 h-10 mt-3 text-xs cursor-pointer md:ml-5 md:mt-0 text-brand-red hover:text-red-300"
              >
                <svg
                  class="w-6 h-6"
                  fill="none"
