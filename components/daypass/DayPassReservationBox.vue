<template>
  <div class="flex flex-col  font-rubik">
    <div class="w-full text-gray-800 rounded-none bg-gray-button">
      <div class="px-3 py-4 text-center text-xl font-bold border-b border-gray-text">
        Your Reservation
      </div>
      <div class="py-6 px-3">
        ...
        <div
          class="flex justify-between px-3 mt-3 item-center"
      ...
      <template v-for="extra in specialPrices">
        <div class="flex items-center text-left justify-between px-6 my-3" :key="extra.id">
          <div>{{ extra.name }}</div>
          <div>
    ...
    <div class="w-full text-gray-800 bg-gray-button" v-if="showDiscount">
      <div class="flex justify-between px-6 py-3 border-b border-t border-gray-text">
        <div>Sub-total</div>
      ...
      <div class="flex justify-between px-6 my-3" v-if="discount > 0">
        <div>Discount</div>
        <div class="font-bold">- {{ currency(discount) }}</div>
      </div>
    </div>

    <div class="w-full px-6 py-3 bg-gray-button">
      <div class="flex justify-between py-3 w-full">
        <div class="">Consumption Tax and VAT (12.5%)</div>
        <div class="">{{ currency(taxTotal) }}</div>
      </div>
    ...
    <div
      ...
      class="w-full px-3 py-3 border rounded-md border-brand-blue-300 bg-brand-blue-100"
    >
...
export default {
  ...
  computed: {
    ...
    selectedOptions() {
      ...
      let filteredSelectedOptions = selectedOptions.filter(
