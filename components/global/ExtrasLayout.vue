<template>
  <div class="relative">
    <SpecialLookout
      v-if="currentSpecial == 'lookout'"
      @next="gotoNext()"
      @prev="gotoPrev()"
    />

    <SpecialMassage
      v-else-if="currentSpecial == 'massage'"
      @next="gotoNext()"
      @prev="gotoPrev()"
    />

    <SpecialNewMassage
      v-else-if="currentSpecial == 'newmassage'"
      @next="gotoNext()"
      @prev="gotoPrev()"
    />

    <SpecialQuadBikes
      v-else-if="currentSpecial == 'quadbike' || currentSpecial == 'quadbikes'"
      @next="gotoNext()"
      @prev="gotoPrev()"
    />

    <SpecialBikes
      v-else-if="currentSpecial == 'bikes'"
      @next="gotoNext()"
      @prev="gotoPrev()"
    />

    <SpecialPhotoshoot
      v-if="currentSpecial == 'photoshoot'"
      @next="gotoNext()"
      @prev="gotoPrev()"
    />

    <SpecialDrinkCollection
      v-if="currentSpecial == 'drinks'"
      @next="gotoNext()"
      @prev="gotoPrev()"
    />

    <SpecialMassagesMultiple
      v-if="currentSpecial == 'massages'"
      @next="gotoNext()"
      @prev="gotoPrev()"
    />

    <SpecialCake
      v-else-if="currentSpecial == 'cakes'"
      @next="gotoNext()"
      @prev="gotoPrev()"
    />

    <SpecialRoomDecoration
      v-else-if="currentSpecial == 'roomDecoration'"
      @next="gotoNext()"
      @prev="gotoPrev()"
    />

    <SpecialUnforgettableExperience
      v-else-if="currentSpecial == 'unforgettableExperience'"
      @next="gotoNext()"
      @prev="gotoPrev()"
    />

    <SpecialDomesticStaff
      v-else-if="currentSpecial == 'domesticStaff'"
      @next="gotoNext()"
      @prev="gotoPrev()"
    />

    <SpecialDayPass
      v-else-if="currentSpecial == 'dayPass'"
      @next="gotoNext()"
      @prev="gotoPrev()"
    />
...
export default {
  ...
  methods: {
    removeExtra() {
      const confirmRemove = confirm("Are you sure you want to remove this Extra?");
    ...
    gotoNext() {
      ...
      if (this.index == this.selected.length - 1) {
        ...
        this.$store.dispatch("extras/updateExtrasSelected", "");
        return;
      }
